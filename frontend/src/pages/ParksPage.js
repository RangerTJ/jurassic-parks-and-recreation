...
function ParksPage ({hostURL}) {
    ...
    const delParks = async (delID) => {
        try {
            if (window.confirm(`Are you sure you want to remove Park #${delID}?`)) {
                await Axios.delete(deleteParksURL + delID);
                
                const mainViewResponse = await Axios.get(getParksURL);
                setParksList(mainViewResponse.data);
                console.log(mainViewResponse.data);
        
                alert(`Park #${delID} has been removed from the database.`);
            }} catch (error) {
